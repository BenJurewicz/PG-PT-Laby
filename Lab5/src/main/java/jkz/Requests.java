...
public class Requests {
    public static List<?>  getCandidatesForRaise(){
        ...
        String sqlQuery = "SELECT e.name AS EmployeeName, d.name AS DepartmentName, e.employedFrom FROM Employee e JOIN Department d ON e.department_id = d.id WHERE e.employment_date = (SELECT MIN(employment_date) FROM Employee WHERE department_id = e.department_id);";
